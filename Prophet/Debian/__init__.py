...
dirs = DebianSet(("/etc/menu", "/usr/lib/menu",
                  "/usr/share/menu", "/usr/share/menu/default", "~/.menu"))
...
def _parse(debmenu):
    try:
        ...
        # encoding= is not supported by Py2, so use codecs.open() as the least
        # common denominator.
...
class App(Prophet.App):
    ...
    def __setup__(self, entry):
        ...
        # To accomplish this scan title/longtitle/description/whatever for the
        # clues (gnome-*, "Calculator for GNOME", etc.)
...
_rejs = (
    Kw("Modules"), Kw("WindowManagers"), Kw(
        "WorkSpace"), Kw("Lock"), Kw("GNOME")
)
