...
class wmaker(Prophet.Legacy.App):
    pass


class wmsetbg(Prophet.Legacy.App):
    exeargs = "-u -t"


class setstyle(Prophet.Legacy.App):
    pass


class seticons(Prophet.Legacy.App):
    pass


class getstyle(Prophet.Legacy.App):
    exeargs = "-t"


class geticonset(Prophet.Legacy.App):
    exeargs = "-t"
...
class AppearanceMenu(MenuMaker.Menu):
    ...
    def __init__(self):
        ...
        try:
            ...
            pass
        try:
            subs += self.openMenu("Icons", seticons().execmd,
                                  [sys + "IconSets", usr + "IconSets"])
        except Prophet.NotSet:
            pass
        try:
            subs += self.openMenu("Backgrounds", wmsetbg().execmd,
                                  [sys + "Backgrounds", usr + "Backgrounds"])
        except Prophet.NotSet:
            pass
        try:
            subs.append(X("Save Theme", "SHEXEC", getstyle(
            ).execmd + " " + usr + '''Themes/\\"%a(Theme name)\\"'''))
        except Prophet.NotSet:
            pass
        try:
            subs.append(X("Save IconSet", "SHEXEC", geticonset(
            ).execmd + " " + usr + '''IconSets/\\"%a(IconSet name)\\"'''))
        except Prophet.NotSet:
            pass
...
class X(MenuMaker.Entry):
    ...
    def emit(self, level):
        if self.arg:
            s = '%s("%s", %s, "%s")' % (
                indent(level), _map(self.name), self.cmd, self.arg)
        else:
