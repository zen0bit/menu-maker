...
class Entry(object):
    ...
    # Element should be placed at the menu top retaining the original order
    StickTop = 1
...
class Menu(Entry, list):
    ...
    def emit(self, level):
        if omitEmptyMenu and self.empty:
            ...
            # Delegating it in hope that there is frontend-specific emit() down
            # the inheritance graph
            return super(Menu, self).emit(level)
